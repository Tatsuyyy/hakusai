...
class ExhibitionRunView(TemplateView):
    ...
    def get_context_data(self, **kwargs: Any) -> Dict[str, Any]:
        ...
        context["project_urls"] = [
            item.url for item in VExhibitionList.objects.filter(exhibitions_id=id)
        ]
    ...
    def post(self, request: http.HttpRequest, *args: Any, **kwargs: Any) -> http.HttpResponse:
        ...
        if data["operation"] == "start":
            ...
            projects = [
                item
                for item in VExhibitionList.objects.filter(exhibitions_id=id).order_by("exec_order")
            ]
        ...
        while True:
            try:
                for project in projects:
                    steps = VProjectSteps.objects.filter(project_id=project.project_id).order_by(
                        "exec_order"
                    )
                    ...
                    for step in steps:
                        ...
                        if self.stop_event.is_set():
                            ...
                            driver.scroll_by_elem(step.xpath)
                        else:
                            pass
                        # 2秒待機
                        time.sleep(2)
